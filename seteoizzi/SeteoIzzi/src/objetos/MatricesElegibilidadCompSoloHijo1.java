/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package objetos;

/**
 *
 * @author hector.pineda
 */
public class MatricesElegibilidadCompSoloHijo1 {
    private String ciudad;
    private String region;
    private String pais;
    private String producto;
    private String tipoRegla;
    private String codigoAccionAsset;
    private String Rowid;
    private String nombrematriz;

    public String getRowid() {
        return Rowid;
    }

    public void setRowid(String Rowid) {
        this.Rowid = Rowid;
    }

    public String getCiudad() {
        return ciudad;
    }

    public void setCiudad(String ciudad) {
        this.ciudad = ciudad;
    }

    public String getRegion() {
        return region;
    }

    public void setRegion(String region) {
        this.region = region;
    }

    public String getPais() {
        return pais;
    }

    public void setPais(String pais) {
        this.pais = pais;
    }

    public String getProducto() {
        return producto;
    }

    public void setProducto(String producto) {
        this.producto = producto;
    }

    public String getTipoRegla() {
        return tipoRegla;
    }

    public void setTipoRegla(String tipoRegla) {
        this.tipoRegla = tipoRegla;
    }

    public String getCodigoAccionAsset() {
        return codigoAccionAsset;
    }

    public void setCodigoAccionAsset(String codigoAccionAsset) {
        this.codigoAccionAsset = codigoAccionAsset;
    }

    public String getNombrematriz() {
        return nombrematriz;
    }

    public void setNombrematriz(String nombrematriz) {
        this.nombrematriz = nombrematriz;
    }
    
    
}
